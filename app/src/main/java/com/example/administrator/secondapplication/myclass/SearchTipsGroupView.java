...
public class SearchTipsGroupView extends LinearLayout {
     ...
     * @param onItemClick
     */
    public void initLabelViews(String items[], final OnItemClick onItemClick) {
        ...
        for (int i = 0; i < size; i++) {//便利items
            ...
            View view = LayoutInflater.from(context).inflate(R.layout.item_search_textview, null);
            ...
            if (length > screenWidth-350) {//当前行的长度大于屏幕宽度则换行
                length = 0;
    ...
    public void initViews(List<String> items, OnItemClick onItemClick) {
        initLabelViews((String[]) items.toArray(), onItemClick);
    }
