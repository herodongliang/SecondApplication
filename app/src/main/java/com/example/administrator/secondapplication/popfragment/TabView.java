package com.example.administrator.secondapplication.popfragment;
...
import com.ycl.tabview.library.*;
...
public class TabView extends RelativeLayout {
    ...
    private void initCustomAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, com.ycl.tabview.library.R.styleable.TabView);
    ...
    private void initCustomAttr(int attr, TypedArray typedArray) {
        if (attr == com.ycl.tabview.library.R.styleable.TabView_tab_textViewSelColor) {
            mTextViewSelColor = typedArray.getColor(attr, mTextViewSelColor);
        } else if (attr == com.ycl.tabview.library.R.styleable.TabView_tab_textViewUnSelColor) {
            mTextViewUnSelColor = typedArray.getColor(attr,mTextViewUnSelColor);
        } else if (attr == com.ycl.tabview.library.R.styleable.TabView_tab_tabViewBackgroundColor) {
            mTabViewBackgroundColor = typedArray.getColor(attr, mTabViewBackgroundColor);
        } else if (attr == com.ycl.tabview.library.R.styleable.TabView_tab_tabViewHeight) {
            mTabViewHeight = typedArray.getDimensionPixelSize(attr, mTabViewHeight);
        } else if (attr == com.ycl.tabview.library.R.styleable.TabView_imageViewTextViewMargin) {
            mImageViewTextViewMargin = typedArray.getDimensionPixelSize(attr, mImageViewTextViewMargin);
        } else if (attr == com.ycl.tabview.library.R.styleable.TabView_tab_textViewSize) {
            mTextViewSize = typedArray.getDimensionPixelSize(attr, mTextViewSize);
        } else if (attr == com.ycl.tabview.library.R.styleable.TabView_tab_imageViewWidth) {
            mImageViewWidth = typedArray.getDimensionPixelSize(attr, mImageViewWidth);
        } else if (attr == com.ycl.tabview.library.R.styleable.TabView_tab_imageViewHeight) {
            mImageViewHeight = typedArray.getDimensionPixelSize(attr, mImageViewHeight);
        } else if (attr == com.ycl.tabview.library.R.styleable.TabView_tab_tabViewGravity) {
            mTabViewGravity = typedArray.getInt(attr, mTabViewGravity);
        } else if (attr == com.ycl.tabview.library.R.styleable.TabView_tab_tabViewDefaultPosition) {
    ...
    private void initView(Context context){
        ...
        tabview.setId(com.ycl.tabview.library.R.id.tabview_id);


        mFragmentContainer=new FrameLayout(context);
        mFragmentContainer.setId(com.ycl.tabview.library.R.id.tabview_fragment_container);
        ...
        if(mTabViewGravity==Gravity.BOTTOM){
            ...
            fragmentContainerParams.addRule(RelativeLayout.ABOVE, com.ycl.tabview.library.R.id.tabview_id);
            ...
            fragmentContainerParams.addRule(RelativeLayout.RIGHT_OF, com.ycl.tabview.library.R.id.tabview_id);
        }else if(mTabViewGravity==Gravity.TOP){
            tabviewParams=new RelativeLayout.LayoutParams(RMP,mTabViewHeight);
            tabview.setOrientation(LinearLayout.HORIZONTAL);
            tabviewParams.addRule(RelativeLayout.ALIGN_PARENT_TOP);
            fragmentContainerParams.addRule(RelativeLayout.BELOW, com.ycl.tabview.library.R.id.tabview_id);
        }else if(mTabViewGravity==Gravity.RIGHT){
            tabviewParams=new RelativeLayout.LayoutParams(mTabViewHeight,RMP);
            tabview.setOrientation(LinearLayout.VERTICAL);
            tabviewParams.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);

            fragmentContainerParams.addRule(RelativeLayout.LEFT_OF, com.ycl.tabview.library.R.id.tabview_id);
        }
    ...
    private void initTabChildView(){
        ...
        if(mTabViewDefaultPosition>=mTabViewChildList.size()){
            mSupportFragmentManager.beginTransaction().add(com.ycl.tabview.library.R.id.tabview_fragment_container, mFragments[0]).show(mFragments[0]).commit();
        }else{
            mSupportFragmentManager.beginTransaction().add(com.ycl.tabview.library.R.id.tabview_fragment_container, mFragments[mTabViewDefaultPosition]).show(mFragments[mTabViewDefaultPosition]).commit();
        }
